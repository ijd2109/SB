...
# get "labels" from the project folder as well:
labels = labels %>% 
  mutate(toMatch = 
           ifelse(wave_to_pull==1,IDENT_SUBID,paste0(IDENT_SUBID,"_fu",wave_to_pull-1)))
rawRowSelect = match(labels$toMatch, rawStructuralData$Measure.volume)
# filter the raw data by choosing the wave1 data:
# also select the columns in varsToKeep
structuralRawData = rawStructuralData[rawRowSelect, varsToKeep]
saveRDS(structuralRawData, "../data/processed/wave1structuralData.rds")
```

# Run PCA
```{r}
#note:
# "SB028" "SB058" "SB102" "SB155" "SB163" "SB201" "SB202" "SB258" "SB282"
# the above participants do not have wave1 structure data; deleting (since other data is wave1)
keepRows = complete.cases(structuralRawData)
brainStructurePCA = prcomp(structuralRawData[keepRows,], 
