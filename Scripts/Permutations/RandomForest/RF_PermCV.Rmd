...
# Get the "baseline" fits to generate OOB scores and variable importances.
...
# RE-RUN ON THE TWO FUNCTIONAL PCA DATASETS

# THEN INSERT INTO THE PERMUTATION RESULTS

# THEN DELETE THIS CHUNK, RERUN ALL OTHER COMPUTATIONS USING `permutationResults`

PCApermRedoResults = mclapply(X = c("FDPCA", "FCPCA"), mc.cores = detectCores() - 1, 
  FUN = function(x) {
    dat = data_list[[x]]; params = param_list[[x]]
    # pre-proc
    dat = na.omit(
...
names(PCApermRedoResults) <- c("FDPCA", "FCPCA")
permutationResults <- readRDS("~/DANL/SB/output/permutations/FINALgridCVPerm.rds")
names(permutationResults) <- names(data_list)
permutationResults[["FDPCA"]] <- PCApermRedoResults[["FDPCA"]]
permutationResults[["FCPCA"]] <- PCApermRedoResults[["FCPCA"]]
# AND THEN OVERWRITE THE PREVIOUS RESULTS:
