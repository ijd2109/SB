...
paste0("masterDfList contains ", length(masterDfList), " elements, each with ", nrow(masterDfList[[1]])," row and ", ncol(masterDfList[[1]]), " columns.")
```
```{r}
...
for (i in lblDataToPull$IDENT_SUBID) {
  ...
  if (any(is.na(CORR))) { #now check for failed correlation computations
    ...
    #compute the correlation matrix anyway, if possible, using 'complete observations' only
...
# I chose to drop the particiipants who had some time series with zero-variance:
